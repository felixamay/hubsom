...
void main() {
  ...
  test('go live does not copy product data-URL photos onto the stream', () async {
    ...
    expect(stream.cover, isEmpty);
    expect(stream.hosts.first.avatar, isEmpty);
  ...
  test('ended lives are pruned and bulky covers stripped on save', () async {
    ...
    expect(listed.every((s) => s.cover.isEmpty), isTrue);
    expect(listed.every((s) => s.hosts.every((h) => h.avatar.isEmpty)), isTrue);
  });
}
